# HANDOFF — 마지막 갱신: 2026-07-31 (장소: 학교)

## 새 세션 시작 시 가장 먼저 할 것
1. 이 파일 전체를 읽는다 (특히 "이번까지 발견된 버그·주의사항" — 같은 실수를 반복하지 않기 위해 필수)
2. `git log --oneline -5`, `git status`로 커밋 상태 확인 (마지막 커밋 `b9958f5`, origin/main과 완전히 동기화됨 — push 확인 완료)
...
5. 민석님에게 "`03_features.ipynb`(1~13절, 86셀)를 민석님이 전체 실행해 에러 0건·수치 일치까지 확인했고, 이중 관점 재검토로 나온 5가지 보완과 12절 잔여 리스크 3가지 기록까지 전부 커밋·push 완료했습니다. 다음은 `04_model_selection.ipynb`를 새로 만들어 로드맵 7단계(모델 선택)를 시작하는 것입니다" 정도로 보고 후 시작
